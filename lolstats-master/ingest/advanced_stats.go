...
func CalculateRolePosition(minionStatTracker *MinionStatTracker, buddyFinder *BuddyFinder) (map[int64]baseview.RolePosition, error) {
	...
	botCS := map[int64]int64{}
	for p, levelMap := range minionStatTracker.MinionInteractions {
		for level, regionMap := range levelMap {
			...
			if val, ok := regionMap[baseview.RegionBot]; ok {
				botCS[p] += val
			}
		}
	}
	...
	// Identify the supports - least attacks bot lane up to level 6 (not counting 0 attacks)
	// Temporarily fill up roles with 0 so we don't count them as the support
	for i := int64(1); i <= 10; i++ {
		if botCS[i] == 0 && roles[i] == "" {
			roles[i] = baseview.RoleAdc
		}
	}

	t1Support := findCs(min, botCS, baseview.BlueTeam, roles)
	t2Support := findCs(min, botCS, baseview.RedTeam, roles)
	roles[t1Support] = baseview.RoleSupport
	roles[t2Support] = baseview.RoleSupport

	for i := int64(1); i <= 10; i++ {
		if roles[i] == baseview.RoleAdc {
			delete(roles, i)
		}
	}

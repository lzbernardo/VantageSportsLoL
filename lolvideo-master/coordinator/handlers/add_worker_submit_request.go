...
type addWorkerSubmitContext struct {
	SpotFleetRequestId *string
}

func (h *LolVideoHandler) HandleAddWorkerSubmitRequest() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		req, err := coordinator.AddWorkers(
			...
			coordinator.Ec2WorkerInstanceTypeVideogen,
			false,
		)
		if err != nil {
			log.Println(err)
